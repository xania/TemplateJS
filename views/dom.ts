...
export class DomDriver implements IDriver {
  ...
  createEvent(name: string, value: Function | Executable<any>) {
    ...
    if (typeof value === "function") target.addEventListener(name, value);
    else target.addEventListener(name, evt => value.execute(evt));
    return {
      dispose() {
...
function createAttribute(target, name: string, value: Primitive | Primitive[]) {
  ...
  function valueAttribute(value: string) {
    if (value === null || value === undefined) target.value = "";
